...
<div class="min-h-screen bg-white text-brand-black">
  ...
  <div class="max-w-6xl mx-auto px-6 py-8">
    <div class="flex items-center justify-between mb-6">
      <h1 class="font-display text-2xl font-bold text-brand-black">Events</h1>
    ...
    {#if events.length === 0}
      <div class="text-center py-20 border border-gray-200 rounded-lg">
        <p class="font-mono text-gray-400 text-sm">No events yet.</p>
      </div>
    {:else}
                ...
                <td class="px-4 py-3">
                  <span class="px-2 py-0.5 rounded-full border text-[10px] uppercase tracking-widest
                    {e.status === 'confirmed' ? 'bg-brand-yellow/10 border-brand-yellow/20 text-green-700' :
                     ...
                     'bg-gray-50 border-gray-200 text-gray-600'}">{e.status}</span>
                </td>
                <td class="px-4 py-3 text-right text-gray-600">{e.brief_count}</td>
                <td class="px-4 py-3 text-right text-gray-600">{e.booking_count}</td>
              </tr>
            {/each}
          </tbody>
        </table>
