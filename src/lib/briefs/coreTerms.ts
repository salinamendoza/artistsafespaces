/**
 * Standard terms included in every Artist Safespaces brief.
 * Prepended to brief-specific terms on the public brief page and
 * captured in `accepted_terms_snapshot` when an artist accepts.
 *
 * To update: edit this constant. Future accepts will snapshot the
 * new version. Existing accepts remain bound to the version they
 * agreed to (stored on the booking).
 */
export const AS_CORE_TERMS = `
## Working with Artist Safespaces — standard terms

**Artist Safespaces (AS) is your sole point of contact for this engagement and the underlying client relationship.** By accepting this brief you agree to the following, in addition to any brief-specific terms below.

1. **All client communication runs through AS.** Scheduling, scope, changes, materials, approvals, invoicing, and any other coordination with the client or their team flows through Artist Safespaces. You will not contact the client directly about this engagement or any related matter without AS's prior written approval, except during on-site introductions facilitated by AS at the event itself.
...
5. **Confidentiality.** The brief, rate, materials, and any non-public information AS shares with you in connection with this engagement (including client names when not disclosed publicly, internal creative direction, and other artists' participation) is confidential. You may share finished work publicly with AS and client credit once the event is public, subject to any brief-specific attribution requirements.
